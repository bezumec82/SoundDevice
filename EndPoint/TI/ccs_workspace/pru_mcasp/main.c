...
struct buffer tx8_bufs [NUM_TX8_SLOTS] [NUM_TX8_BUFS];
    ...
    struct mcasp mcasp8_tx = {
        ...
        .bufs_ddr_start_addr        = MCASP8_TX_BUFS_START_ADDR,
...
struct buffer rx8_bufs [NUM_RX8_SLOTS] [NUM_RX8_BUFS];
    ...
    struct mcasp mcasp8_rx = {
        ...
        .bufs_ddr_start_addr        = MCASP8_RX_BUFS_START_ADDR,
        .buf_size_bytes             = BUF_SIZE_BYTES,
...
struct buffer tx1_bufs [NUM_TX1_SLOTS] [NUM_TX1_BUFS];
    ...
    struct mcasp mcasp1_tx = {
        ...
        .bufs_ddr_start_addr        = MCASP1_TX_BUFS_START_ADDR,
        .buf_size_bytes             = BUF_SIZE_BYTES,
        .buf_size_words             = BUF_SIZE_WORDS,
...
struct buffer rx1_bufs [NUM_RX1_SLOTS] [NUM_RX1_BUFS];
    ...
    struct mcasp mcasp1_rx = {
        ...
        .bufs_ddr_start_addr        = MCASP1_RX_BUFS_START_ADDR,
        .buf_size_bytes             = BUF_SIZE_BYTES,
...
static inline void init_bufs ( struct mcasp * mcasp_ptr )
{
    ...
    for (i = 0; i < mcasp_ptr->num_slots; i++)
    {
        ...
        for (j = 0; j < mcasp_ptr->num_bufs_per_slot - 1; j++)
        {
            ...
            bufs->buf           = (uint32_t *)      ( mcasp_ptr->bufs_ddr_start_addr +
...
/* payload receives RPMsg message with DDR start address */
uint32_t ddr_start_addr = 0xffffffff;
...
int main(void)
{
    ...
    /*--------------------------------------*/
        ...
        init_bufs(&mcasp8_tx);
    #else
        uint32_t tx8_bufs_ddr_start_addr = MCASP8_TX_BUFS_START_ADDR; ///TO DO : receive this address from mailbox
        for (i = 0; i < NUM_TX8_SLOTS; i++)
        {
            for (j = 0; j < NUM_TX8_BUFS; j++)
            {
                tx8_bufs[i][j].next      = &tx8_bufs[i][j+1];
                tx8_bufs[i][j].buf       = (uint32_t *) (   tx8_bufs_ddr_start_addr +
    ...
    /*--------------------------------------*/
        ...
        init_bufs(&mcasp8_rx);
    #else
        uint32_t rx8_bufs_ddr_start_addr = MCASP8_RX_BUFS_START_ADDR;    ///TO DO : receive this address from mailboxf
        for (i = 0; i < NUM_RX8_SLOTS; i++)
        {
            for (j = 0; j < NUM_RX8_BUFS; j++)
            {
                rx8_bufs[i][j].next      = &rx8_bufs[i][j+1];
                rx8_bufs[i][j].buf       = (uint32_t *) (   rx8_bufs_ddr_start_addr +
    ...
    /*--------------------------------------*/
        ...
        init_bufs(&mcasp1_tx);
    #else
        uint32_t tx1_bufs_ddr_start_addr = MCASP1_TX_BUFS_START_ADDR; ///TO DO : receive this address from mailbox
        for (i = 0; i < NUM_TX1_SLOTS; i++)
        {
            for (j = 0; j < NUM_TX1_BUFS; j++)
            {
                tx1_bufs[i][j].next      = &tx1_bufs[i][j+1];
                tx1_bufs[i][j].buf       = (uint32_t *) (   tx1_bufs_ddr_start_addr +
    ...
    /*--------------------------------------*/
        ...
        init_bufs(&mcasp1_rx);
    #else
        uint32_t rx1_bufs_ddr_start_addr = MCASP1_RX_BUFS_START_ADDR;    ///TO DO : receive this address from mailbox
        for (i = 0; i < NUM_RX1_SLOTS; i++)
        {
            for (j = 0; j < NUM_RX1_BUFS; j++)
            {
                rx1_bufs[i][j].next      = &rx1_bufs[i][j+1];
                rx1_bufs[i][j].buf       = (uint32_t *) (   rx1_bufs_ddr_start_addr +
