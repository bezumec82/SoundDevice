...
static ulong spl_net_load_read(
                            ...
                            void * buf)
{
    ...
    printf("%s: (!?)memcpy(buf:0x%lx, (laddr : 0x%lx + sector : 0x%lx), count:%ld)\n",
          __func__,(ulong)buf, load_addr, sector, count);
    if (buf != (void *)(load_addr + sector))
        memcpy(buf, (void *)(load_addr + sector), count);
    else
        puts("memcpy unnecessary\n");
    return count;
}
