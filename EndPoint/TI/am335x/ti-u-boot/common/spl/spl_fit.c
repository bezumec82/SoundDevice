                            ...
                            struct spl_image_info * image_info)
{
    ...
    if (external_data) {
        ...
        src = (void *)data;
        debug("Embedded data: node : %d, src : 0x%lx, load_addr : 0x%lx, length : %ld\n", 
               node, (ulong)src, load_addr, (unsigned long)length);
    }
    ...
    if (IS_ENABLED(CONFIG_SPL_GZIP) && image_comp == IH_COMP_GZIP) {
        ...
        printf("%s : (!)memcpy(load_addr : 0x%lx, src : 0x%lx, length : %ld)\n", 
              __func__, (ulong) load_addr, (ulong)src, (ulong)length);
                        ...
                        void *                  fit)
{
    ...
    debug("fit read sector %lx, sectors=%d, dst=0x%lx, count=%lu, size=0x%lx\n",
          sector, sectors, (ulong)fit, count, size);
    ...
    /* Load the image and set up the spl_image structure */
    ret = spl_load_fit_image(info, sector, fit, base_offset, node, spl_image);
    if (ret)
        return ret;
