 ...
 * @{ */
#ifndef __KERNELPRINTKDEBUG__
#define __KERNELPRINTKDEBUG__
...
#define DEBUG_ATM_PCTL_SEVERE       DISABLE
#define DEBUG_ATM_PCTL              DISABLE
...
struct atmel_pioctrl {
    void __iomem *          reg_base;           ///< Base address of the controller.
    struct clk *            clk;                ///< Clock of the controller.
    unsigned                nbanks;             ///< Number of PIO groups, it can vary depending on the SoC.
    struct pinctrl_dev *    pinctrl_dev;        ///< Pinctrl device registered.
    struct atmel_group *    groups;             ///< Groups table to provide group name and pin in the group to pinctrl.
    const char * const *    group_names;        ///< Group names table to provide all the group/pin names to pinctrl or gpio.
    struct atmel_pin **     pins;               ///< Pins table used for both pinctrl and gpio. 'pin_id', 'bank' and 'line' fields are set at probe time. Other ones are set when parsing dt pinctrl.
    unsigned                npins;              ///< Number of pins.
    struct gpio_chip *      gpio_chip;          ///< Gpio chip registered.
    struct irq_domain *     irq_domain;         ///< IRQ domain for the gpio controller.
    int *                   irqs;               ///< Table containing the hw irq number of the bank. The index of the table is the bank id.
    ...
    struct device       *   dev;                ///< Device entry for the Atmel PIO controller.
    struct device_node  *   node;               ///< Node of the Atmel PIO controller.
};
...
/* --- GPIO --- */
static unsigned int atmel_gpio_read (
...
{
    /*
     * Nothing to do, interrupt is cleared when reading the status register.
     */
}
...
static void         atmel_gpio_irq_handler(struct irq_desc * desc)
{
    ...
    if (bank < 0)
    {
        __PRINTK(RED, "ERROR : No bank associated to irq %u\n", irq);
        return;
    }
    ...
    for (;;)
    {
        isr = (unsigned long)atmel_gpio_read(
                                            atmel_pioctrl,
                                            bank,
                                            ATMEL_PIO_ISR
                                            );
...
static int          atmel_gpio_direction_output (
...
/* --- PINCTRL --- */
static unsigned int atmel_pin_config_read(struct pinctrl_dev * pctldev, unsigned pin_id )
{
    struct atmel_pioctrl *  atmel_pioctrl   = pinctrl_dev_get_drvdata(pctldev);
    unsigned                bank            = atmel_pioctrl->pins[pin_id]->bank;
    unsigned                line            = atmel_pioctrl->pins[pin_id]->line;
    void __iomem *          addr            = atmel_pioctrl->reg_base + bank * ATMEL_PIO_BANK_OFFSET;
...
static void         atmel_pin_config_write(struct pinctrl_dev * pctldev, unsigned pin_id, u32 conf)
{
    struct atmel_pioctrl *  atmel_pioctrl   = pinctrl_dev_get_drvdata(pctldev);
    unsigned                bank            = atmel_pioctrl->pins[pin_id]->bank;
    unsigned                line            = atmel_pioctrl->pins[pin_id]->line;
...
static const char * atmel_pctl_get_group_name(struct pinctrl_dev * pctldev, unsigned selector)
{
    struct atmel_pioctrl * atmel_pioctrl = pinctrl_dev_get_drvdata(pctldev);
    return atmel_pioctrl->groups[selector].name;
}

static int          atmel_pctl_get_group_pins   (
...
static struct atmel_group * atmel_pctl_find_group_by_pin(struct pinctrl_dev * pctldev, unsigned pin)
{
    struct atmel_pioctrl * atmel_pioctrl = pinctrl_dev_get_drvdata(pctldev);
    int i;
                                            ...
                                            )
{
    ...
    {
        __PRINTK(RED,   "ERROR : Node '%s'. Pin %u has function %u.\n",
                        of_node_full_name(np), pin_id, func_id);
        return -EINVAL;
    }
    ...
    *func_name = atmel_functions[func_id];
    ...
    if (!grp)
    {
        __PRINTK(RED,   "ERROR : Node '%s'. Can't find group for pin %u.\n",
                        of_node_full_name(np), pin_id);
        return -EINVAL;
    }
    *grp_name = grp->name;
...
static int          atmel_pctl_dt_subnode_to_map(
                                                struct pinctrl_dev *    pctldev,
                                                struct device_node *    np,
                                                ...
                                                )
{
    ...
    if (!pins) { return -EINVAL; }

    ret = pinconf_generic_parse_dt_config(np, pctldev, &configs, &num_configs);
    if (ret < 0)
    {
        __PRINTK(RED, "ERROR : Could not parse node property.\n");        
        return ret;
    }

    if (num_configs) { has_config = true; }

    num_pins = pins->length / sizeof(u32);
    if (!num_pins)
    {
        __PRINTK(RED, "ERROR : No pins found in node %s.\n", of_node_full_name(np));            
        ret = -EINVAL;
        goto exit;
    }
     ...
     */
    reserve = 1;
    if (has_config && num_pins >= 1) { reserve++; }
    reserve *= num_pins;
    ret = pinctrl_utils_reserve_map (
                                    pctldev,
                                    map,
                                    reserved_maps,
                                    num_maps,
    ...
    if (ret < 0)
    {
        __PRINTK(RED, "ERROR : Can't reserve map for node %s.\n", of_node_full_name(np)); 
        goto exit;
    }
    ...
    for (i = 0; i < num_pins; i++)
    {
        ...
        if (ret)
        {
            __PRINTK(RED, "ERROR : Can't read 'pinmux' property of node %s.\n", of_node_full_name(np));
            goto exit;
        }
        ret = atmel_pctl_xlate_pinfunc  (
                                        pctldev,
                                        np,
                                        pinfunc,
                                        &group,
...
static int          atmel_pctl_dt_node_to_map   (
                                                struct pinctrl_dev *    pctldev,
                                                ...
                                                )
{
    ...
    *num_maps       = 0;
     ...
     * it is useless to add a subnode, so directly parse node referenced by phandle.
     */
    ret = atmel_pctl_dt_subnode_to_map  (
                                        pctldev,
                                        np_config,
                                        map,
                                        &reserved_maps,
                 ...
                 unsigned group)
{
    ...
    u32                     conf;
#if DEBUG_ATM_PCTL
    __PRINTK(CYN, "Enable function '%s' for pin '%s'.\n",
    ...
    //__PRINTK(CYN, "pin=%u, conf: 0x%08x\n", pin, conf);
...
static int atmel_conf_pin_config_group_get  (
                                            struct pinctrl_dev *    pctldev,
                                            unsigned                group,
...
#if DEBUG_ATM_PCTL_SEVERE
        __PRINTK(CYN, "pin=%u, config=0x%lx\n", pin_id, configs[i]);
#endif
        switch (param) {
        case PIN_CONFIG_BIAS_DISABLE:
...
static int  atmel_pinctrl_probe (struct platform_device *pdev)
{
    ...
    if (!atmel_pioctrl) { return -ENOMEM; }
    ...
    if (!match)
    {
        __PRINTK(RED, "ERROR : Unknown 'compatible' string.\n");
        return -ENODEV;
    }
    ...
    if (!res)
    {
        __PRINTK(RED, "ERROR : Unable to get Atmel pinctrl IO memory.\n");
        return -EINVAL;
    }
    ...
    {
        __PRINTK(RED, "ERROR : Failed to get clock.\n");
        return PTR_ERR(atmel_pioctrl->clk);
    }

    atmel_pioctrl->pins = devm_kzalloc(dev, sizeof(*atmel_pioctrl->pins)*atmel_pioctrl->npins, GFP_KERNEL);
    if (!atmel_pioctrl->pins) { return -ENOMEM; }

    pin_desc = devm_kzalloc(dev, sizeof(*pin_desc)*atmel_pioctrl->npins, GFP_KERNEL);
    if (!pin_desc) { return -ENOMEM; }
    ...
    group_names = devm_kzalloc(dev, sizeof(*group_names)*atmel_pioctrl->npins, GFP_KERNEL);
    if (!group_names) { return -ENOMEM; }

    atmel_pioctrl->group_names  = group_names;
    atmel_pioctrl->groups       = devm_kzalloc(&pdev->dev, sizeof(*atmel_pioctrl->groups)*atmel_pioctrl->npins, GFP_KERNEL);
    if (!atmel_pioctrl->groups) { return -ENOMEM; }
    ...
    for (i = 0 ; i < atmel_pioctrl->npins; i++)
    {
        ...
        atmel_pioctrl->pins[i] = devm_kzalloc(dev, sizeof(**atmel_pioctrl->pins), GFP_KERNEL);
        if (!atmel_pioctrl->pins[i]) { return -ENOMEM; }
        ...
        pin_desc[i].name = kasprintf(GFP_KERNEL, "P%c%d",  bank + 'A', line);
    ...
    atmel_pioctrl->pm_wakeup_sources = devm_kzalloc(dev, sizeof(*atmel_pioctrl->pm_wakeup_sources)*atmel_pioctrl->nbanks, GFP_KERNEL);
    if (!atmel_pioctrl->pm_wakeup_sources) { return -ENOMEM; }

    atmel_pioctrl->pm_suspend_backup = devm_kzalloc(dev, sizeof(*atmel_pioctrl->pm_suspend_backup)*atmel_pioctrl->nbanks, GFP_KERNEL);
    if (!atmel_pioctrl->pm_suspend_backup) { return -ENOMEM; }

    atmel_pioctrl->irqs = devm_kzalloc(dev, sizeof(*atmel_pioctrl->irqs) * atmel_pioctrl->nbanks, GFP_KERNEL);
    if (!atmel_pioctrl->irqs) { return -ENOMEM; }
    ...
    atmel_pioctrl->irq_domain = irq_domain_add_linear(dev->of_node, atmel_pioctrl->gpio_chip->ngpio, &irq_domain_simple_ops, NULL);
    if (!atmel_pioctrl->irq_domain)
    {
        __PRINTK(RED, "ERROR : Can't add the irq domain.\n");
        return -ENODEV;
    }
...
#if DEBUG_ATM_PCTL_SEVERE
        __PRINTK(MAG, "Atmel gpio irq domain: hwirq: %d, linux irq: %d.\n", i, irq);    
#endif    
    } //end for
    ...
    if (ret)
    {
        __PRINTK(RED, "ERROR : Failed to prepare and enable clock.\n");
        goto clk_prepare_enable_error;
    }

    atmel_pioctrl->pinctrl_dev = devm_pinctrl_register  (
                                                        &pdev->dev,
                                                        &atmel_pinctrl_desc,
                                                        atmel_pioctrl
                                                        );
    if (IS_ERR(atmel_pioctrl->pinctrl_dev))
    {
        ...
        __PRINTK(RED, "ERROR : Pinctrl registration failed.\n");
        goto clk_unprep;
    }
    ...
    if (ret)
    {
        __PRINTK(RED, "ERROR : Failed to add gpiochip.\n");
        goto clk_unprep;
    }

    ret = gpiochip_add_pin_range(
    ...
    if (ret) {
        __PRINTK(RED, "ERROR : Failed to add gpio pin range.\n");
        goto gpiochip_add_pin_range_error;
    }
    __PRINTK(MAG, "Atmel pinctrl initialized.\n");
    return 0;
