...
#define DISABLE                     0
#define ENABLE                      1
#define DEBUG_SDHC_SEVERE           DISABLE
...
{
#if DEBUG_SDHC_SEVERE
    __PRINTK(RED, "Disable SDHCI clock.\n");
#endif
...
{
    int ret;
    //__PRINTK(YEL, "Enable SDHCI clock.\n");
...
static int sdhci_at91_suspend(struct device *dev)
{
    ...
    int ret;
#if DEBUG_SDHC_SEVERE
    __PRINTK(YEL, "Suspend SDHC device.\n");
#endif
...
#if DEBUG_SDHC_SEVERE
    __PRINTK(YEL, "Runtime suspend SDHC device.\n");
#endif
...
static int  sdhci_at91_runtime_resume(struct device *dev)
{
...
#if DEBUG_SDHC_SEVERE
    __PRINTK(GRN, "Resume SDHC device.\n");
#endif
    ...
    if (ret) {
        __PRINTK(RED, "Can't enable clocks.\n");
        return ret;
    }
...
static int sdhci_at91_probe(struct platform_device *pdev)
{
    ...
    if (IS_ERR(priv->mainck)) {
        __PRINTK(RED, "Failed to get 'baseclk'.\n");
        return PTR_ERR(priv->mainck);
    }
    priv->hclock = devm_clk_get(&pdev->dev, "hclock");
    if (IS_ERR(priv->hclock)) {
        __PRINTK(RED, "Failed to get 'hclock'.\n");
        return PTR_ERR(priv->hclock);
    }
    priv->gck = devm_clk_get(&pdev->dev, "multclk");
    if (IS_ERR(priv->gck)) {
        __PRINTK(RED, "Failed to get 'multclk'.\n");
        return PTR_ERR(priv->gck);
    }

    ret = sdhci_at91_set_clks_presets(&pdev->dev);
    if (ret) { goto sdhci_pltfm_free; }

    priv->restore_needed = false;

    ret = mmc_of_parse(host->mmc);
    if (ret) { goto clocks_disable_unprepare; }
     ...
     */
    if (mmc_card_is_removable(host->mmc) && mmc_gpio_get_cd(host->mmc) < 0) 
    {
