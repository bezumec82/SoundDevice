...
#define to_clk_sam9x5_peripheral(hw) container_of(hw, struct clk_sam9x5_peripheral, hw)
...
static int clk_peripheral_enable(struct clk_hw *hw)
{
    ...
    __PRINTK(GRN, "Enable clock for for PID %u.\n", id);
    regmap_write    (
                    periph->regmap,
                    offset,
                    PERIPHERAL_MASK(id)
...
static void clk_peripheral_disable(struct clk_hw *hw)
{
    ...
    __PRINTK(RED, "Disable clock for for PID %u.\n", id);
    regmap_write(
                periph->regmap,
                offset,
                PERIPHERAL_MASK(id)
                                ...
                                )
{
    ...
    if (!periph) { return ERR_PTR(-ENOMEM); }
...
static int clk_sam9x5_peripheral_enable(struct clk_hw * hw)
{
    ...
    if (periph->id < PERIPHERAL_ID_MIN)
        return 0;
#if DEBUG_PERIPH_CLK_SEVERE
    __PRINTK(YEL, "Enable clock for PID %u.\n", periph->id);
#endif
...
of_at91_clk_periph_setup(struct device_node *np, u8 type)
{
    ...
    __PRINTK(MAG, "+++ Initializing peripheral clock +++\n");
    ...
    for_each_child_of_node(np, periphclknp) {
        ...
        __PRINTK(CYN,   "Registering clock : \n"
                        "%49s name = %s. PID = %u. parent_name = %s. \n",
                        "", name, id, parent_name);
#endif
            hw = at91_clk_register_sam9x5_peripheral(
                                                    regmap,
                                                    &pmc_pcr_lock,
                                                    name,
                                                    parent_name,
