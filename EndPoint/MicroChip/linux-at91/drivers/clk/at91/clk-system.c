...
static int          clk_system_prepare(struct clk_hw *hw)
{
    ...
    __PRINTK(YEL,   "Enable system clock ID %i.\n", sys->id);
	regmap_write    (
                    sys->regmap,
                    AT91_PMC_SCER,
                    1 << sys->id
	...
	if (!is_pck(sys->id))
    {
        __PRINTK(GRN""BOLD,"Clock ID %i enabled.", sys->id);
		return 0;
    }
...
{
	struct clk_system * sys = to_clk_system(hw);
    __PRINTK(RED,   "Disable system clock ID %i.\n", sys->id);
	regmap_write    (
                    sys->regmap,
                    AT91_PMC_SCDR,
                    1 << sys->id
                            ...
                            )
{
	...
	if (!parent_name || id > SYSTEM_MAX_ID){
        __PRINTK(RED, "ERROR : ID=%i.\n", id);
		return ERR_PTR(-EINVAL);
    }

	sys = kzalloc(sizeof(*sys), GFP_KERNEL);
	if (!sys) { return ERR_PTR(-ENOMEM); }
...
static void __init of_at91rm9200_clk_sys_setup(struct device_node *np)
{
    ...
    __PRINTK(MAG, "+++ Initializing system clock +++\n");
	...
	for_each_child_of_node(np, sysclknp) {
        ...
        __PRINTK(CYN,   "Registering clock : \n"
                        "%49s name = %s. bit in PMC_SCER = %u. parent_name = %s. \n",
                        "", name, id, parent_name);
